...
class WWC_Admin_Labels {
	...
	private static function render_danger_zone( $queued, $ai_labeled = 0 ) {
		...
		printf(
			'<label style="display:block;margin:8px 0;"><input type="checkbox" name="include_human_verified" value="1" id="wwc-include-human-verified" /> %s</label>',
			esc_html__( 'Also reset products a human wrote or verified by hand — a true fresh start with no exceptions. Cannot be undone; that content cannot be regenerated by AI.', 'wellness-chatbot' )
		);
		printf(
	 ...
	 * scratch. By default the backend still refuses to touch a product a
	 * human wrote/verified themselves; the "include_human_verified" checkbox
	 * on the form is the explicit, opt-in override for a true no-exceptions
	 * reset — this handler just passes it through as-is.
	 */
	public static function handle_reset_all_labels() {
		WWC_Admin::verify_post( 'wwc_reset_all_labels' );

		$include_human_verified = ! empty( $_POST['include_human_verified'] );

		$response = WWC_Backend_Client::post(
			'/api/admin/labels/reset-all',
			array(
				'confirm'                => true,
				'include_human_verified' => $include_human_verified,
			),
			array( 'timeout' => 90 )
		);
